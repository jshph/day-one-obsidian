...
function stripEntryText(raw) {
  const withoutFrontmatter = raw.replace(/^---\n[\s\S]*?\n---\n?/, "");
  return withoutFrontmatter.split("\n").map((line) => {
    const original = line.trim();
    const clean = original.replace(/^[-*>]\s*/, "").replace(/[*_`#]/g, "").trim();
    return { original, clean };
  }).filter(({ original, clean }) => clean && !original.startsWith("#") && !original.startsWith("<!--") && !clean.startsWith("![[") && !/^(📍|🌤|☀️|🌧|🏷|⭐)/u.test(clean)).map(({ clean }) => clean);
}
...
var DayOneShellView = class extends import_obsidian.ItemView {
  ...
  railButton(parent, icon, label, onClick, trailing, active = false) {
  ...
  renderJournalRail(rail) {
    ...
    this.railButton(nav, "sunrise", "Today", async () => {
      this.filter = "today";
      this.mode = "list";
      await this.plugin.openDate((0, import_moment.default)());
      await this.render();
    }, void 0, this.filter === "today");
    this.railButton(nav, "circle-ellipsis", "More", () => {
      this.filter = "all";
      this.mode = "list";
      this.render();
    });
    const onThisDayCount = this.entries.filter((e) => e.date.format("MM-DD") === (0, import_moment.default)().format("MM-DD") && e.date.year() !== (0, import_moment.default)().year()).length;
    ...
    add.onclick = () => new import_obsidian.Notice("This vault starts with one Journal. Add folders whenever you actually need them.");
    ...
    this.railButton(lower, "trash-2", "Trash", () => new import_obsidian.Notice("Deleted notes remain in Obsidian\u2019s .trash folder."), "0");
    this.railButton(lower, "settings", "Settings", () => {
      var _a;
      return (_a = this.app.setting) == null ? void 0 : _a.open();
    });
    this.railButton(lower, "map-pin", "Location History", () => {
      this.mode = "map";
      this.renderBrowser();
    });
  }
  filteredEntries() {
    ...
    if (this.filter === "today") result = result.filter((e) => e.date.isSame((0, import_moment.default)(), "day"));
    if (this.filter === "on-this-day") result = result.filter((e) => e.date.format("MM-DD") === (0, import_moment.default)().format("MM-DD") && e.date.year() !== (0, import_moment.default)().year());
    const q = this.search.trim().toLowerCase();
    if (q) result = result.filter((e) => {
      var _a;
      return `${e.title} ${e.preview} ${(_a = e.location) != null ? _a : ""}`.toLowerCase().includes(q);
    });
    return result;
  }
  renderBrowser() {
  ...
  renderBrowserToolbar(parent) {
    const toolbar = parent.createDiv({ cls: "day-one-browser-toolbar" });
    const modes = toolbar.createDiv({ cls: "day-one-mode-switcher" });
    this.iconButton(modes, "list", "List", () => {
      this.mode = "list";
      this.renderBrowser();
    }, this.mode === "list");
    this.iconButton(modes, "layout-grid", "Photos", () => {
      this.mode = "grid";
      this.renderBrowser();
    }, this.mode === "grid");
    this.iconButton(modes, "map", "Map", () => {
      this.mode = "map";
      this.renderBrowser();
    }, this.mode === "map");
    this.iconButton(modes, "calendar-days", "Calendar", () => {
      this.mode = "calendar";
      this.renderBrowser();
    }, this.mode === "calendar");
    ...
    this.iconButton(tools, "list-filter", "Filter", () => {
      this.filter = this.filter === "all" ? "today" : "all";
      this.render();
    }, this.filter !== "all");
  }
  renderList(parent, entries) {
    var _a;
  ...
  renderMap(parent, entries) {
    ...
    if (!mapped.length) {
      ...
      (0, import_obsidian.setIcon)(pin, "map-pin");
...
var DayOneShellPlugin = class extends import_obsidian.Plugin {
  ...
  updateEditorDates() {
    window.setTimeout(() => {
      ...
      for (const leaf of this.app.workspace.getLeavesOfType("markdown")) {
        ...
        if (!header) continue;
        let label = header.querySelector(".day-one-editor-date");
  ...
  async ensureDailyNote(date) {
    const path = (0, import_obsidian.normalizePath)(`${DAILY_FOLDER}/${date.format("YYYY-MM-DD")}.md`);
